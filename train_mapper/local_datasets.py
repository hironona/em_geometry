...
class ActivationDataset(Dataset):
    ...
    def __getitem__(self, idx) -> dict:
        ...
        modelA_decoded_tokens = [
            tknzA.decode(tok) for tok in tknzA.encode(text, add_special_tokens=False)
        ]
        modelB_decoded_tokens = [
            tknzB.decode(tok) for tok in tknzB.encode(text, add_special_tokens=False)
        ]
        ...
        assert len(modelA_pre_space_token_idx) == len(modelB_pre_space_token_idx), f"""
            ModelA:\n
                decoded tokens: {modelA_decoded_tokens}\n
                token indices: {modelA_pre_space_token_idx}\n
            ModelB:\n
                decoded tokens: {modelB_decoded_tokens}\n
                token indices: {modelB_pre_space_token_idx}
        """
